...
export class DynamicArray {
  ...
  pushUint8(value: number) {
    ...
    // console.log('push u8', value);
    this.view.setUint8(this._length, value);
    this._length += 1;
  }

  getUint32(index: number) {
    return this.view.getUint32(index, true);
  }
  ...
  popUint32() {
    this._length -= 4;
    // console.log('pop u32', this.view.getUint32(this._length, true));
    return this.view.getUint32(this._length, true);
  }

  popFloat32() {
    this._length -= 4;
    // console.log('pop f32', this.view.getFloat32(this._length, true));
    return this.view.getFloat32(this._length, true);
  }

  popUint8() {
    this._length -= 1;
    // console.log('pop u8', this.view.getUint8(this._length));
    return this.view.getUint8(this._length);
  }

  setFrom(array: DynamicArray, srcIndex: number, destIndex: number, length: number) {
    this.array.set(array.array.subarray(srcIndex, srcIndex + length), destIndex);
  }

  pushFrom(array: DynamicArray, srcIndex: number, length: number) {
    if (this._length + length > this.array.length) this.resize(this._length + length);

    this.array.set(array.array.subarray(srcIndex, srcIndex + length), this._length);
    this._length += length;
  }

  popFrom(array: DynamicArray, destIndex: number, length: number) {
    array._length -= length;
    this.array.set(array.array.subarray(array._length, array._length + length), destIndex);
  }

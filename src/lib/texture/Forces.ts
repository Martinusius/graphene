...
import type { Compute, ComputeProgram, ComputeTexture, SpecialComputeProgram } from "./Compute";
...
export class Forces {
  ...
  private attractProgram: SpecialComputeProgram;
  ...
  constructor(private algorithm: ForceAlgorithm, compute: Compute, vertexCount: number, private vertexData: ComputeTexture, private edgeData: ComputeTexture) {
    ...
    this.attractProgram = compute.createSpecialProgram(algorithm.attractionFunction() + attract, true);
  ...
  async update(delta: number) {
    ...
    const hashTableSize = this.hashTable.width * this.hashTable.height * 4;
    ...
    this.attractProgram.execute(2 * this.edgeData.width * this.edgeData.height, this.velocities);
